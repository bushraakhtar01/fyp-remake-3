...
class UI extends Component {
  ...
  async componentDidMount() {
    ...
    // inserting specific header category data
      ...
      (this.state.brands.map(brand=>(brand.size_type==="no size") ? (this.state.specificCat.push(brand)):('hi')
    ))
...
<Link to="/unstitched" className="navlinks"><li  onClick={function(){
        ...
        {/* filter by category */}
            ...
            <img src="unstitchedcover1.jpg" className="topImage"></img>
           
              <h2 className="brandheadingimgcol mb-0"  >{this.state.brands[0].brand}</h2>
              <h3 className="stitched_unstitched">Unstitched</h3>
