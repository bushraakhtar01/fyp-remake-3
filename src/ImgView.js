...
class ImgView extends Component{
 ...
 componentDidMount(){
    ...
    //    if(this.state.propsquantity>quan.value){
  ...
  addtocart(){
      if(this.state.user){
        document.body.scrollTop = 0;
        document.documentElement.scrollTop = 0
        var a=document.querySelector('.modal-body')
...
<img src={this.state.propsimg} className="modalimg"></img>
        ...
        <p className="modalinfo">Cart Subtotal: </p>
   
        <button className="btn modalcheckoutbutton  px-lg-5 px-md-5 px-sm-5 px-2  mt-4 pt-2 pb-2" 
        onClick={this.addtocart}>Proceed to checkout</button>
                               </div>
      </div>
         </div>
